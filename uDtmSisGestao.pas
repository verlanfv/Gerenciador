...
uses
  SysUtils, Classes, {DBXpress,} FMTBcd, DB, DBClient, TConnect, SqlExpr, Provider,
  ...
  Data.DBXMySQL, ppDesignLayer, ppParameter, uFuncoes, Vcl.Dialogs,
...
type
  TdtmSisGestao = class(TDataModule)
    ...
    pplRelDevedor: TppDBPipeline;
    pprRelDevedor: TppReport;
    ...
    lblNomeouRazaoRelatorio: TppLabel;
    ...
    FDQryDevedor: TFDMemTable;
    FDQryDevedorDiasDevedor: TIntegerField;
    FDQryDevedorCodigo: TIntegerField;
    FDQryDevedorNome: TStringField;
    FDQryDevedorSaldoDevedor: TFloatField;
    FDQryDevedorTelefone: TStringField;
    FDQryDevedorEmpresa: TIntegerField;
...
begin
 // Localizar Configurador da NF-e
            ...
            dtmSisGestao.FdQryGeralDados.SQL.Text := LowerCase('SELECT * FROM GERAL_NFE WHERE (empre_codigo = :emprecodigo)');
...
begin
     ...
     dtmSisGestao.FdQryGeral.SQL.Text := LowerCase(sSQLRetornaFuncionario);
...
begin
     ...
     dtmSisGestao.FdQryGeral.Active := False;
     dtmSisGestao.FdQryGeral.SQL.Clear;
     dtmSisGestao.FdQryGeral.SQL.Text := LowerCase(aSQLVerificaAcessoAosMenus);
     ...
     begin
          dtmSisGestao.FdQryGeralDados.Active := False;
          dtmSisGestao.FdQryGeralDados.SQL.Clear;
          dtmSisGestao.FdQryGeralDados.SQL.Text := LowerCase(sSQLVerificaMenu);
          ...
          dtmSisGestao.FdQryGeralDados.Active := False;
          dtmSisGestao.FdQryGeralDados.SQL.Clear;
          dtmSisGestao.FdQryGeralDados.SQL.Text := LowerCase(sSQLRetornaNivelAcesso);
...
begin
     ...
     FdQryGeralDados.SQL.Text := LowerCase('select * from geral_empresa where empre_codigo = 0');
     ...
     begin
          FdQryGeralDados.Active := False;
          FdQryGeralDados.SQL.Clear;
          FdQryGeralDados.SQL.Text := LowerCase(
     ...
     FdQryGeralDados.Active := False;
     FdQryGeralDados.SQL.Clear;
     FdQryGeralDados.SQL.Text := LowerCase('select func_codigo,func_nome '+
     ...
     FdQryGeral.SQL.Text := lowercase('insert into geral_acesso(func_codigo,empre_codigo,menu_codigo,acesso_nivel) '+
     ...
     Application.ProcessMessages;
     FdQryGeralDados.SQL.Clear;
     FdQryGeralDados.SQL.Text := LowerCase('select * from geral_menu');
     Application.ProcessMessages;
     FdQryGeralDados.Active := True;
...
BEGIN
     //Le dados do arquivo INI...
     tfArquivoIni                  := TIniFile.Create(sCaminhoIni);
     ARQUIVOINI.CAMINHOBASE        := tfArquivoIni.ReadString('SISGESTAO','CaminhoBase','X');
     ARQUIVOINI.SERVIDOR           := tfArquivoIni.ReadString('SISGESTAO','Servidor','X');
     ARQUIVOINI.CORFUNDO           := tfArquivoIni.ReadInteger('SISGESTAO','CorFundo',0);
     ARQUIVOINI.CAMINHOSESSAO      := tfArquivoIni.ReadString('SISGESTAO','CaminhoSessao','X');
     ARQUIVOINI.USUARIO            := Decripta_Email(Trim(tfArquivoIni.ReadString('SISGESTAO','usuario','X')));
     ARQUIVOINI.SENHA              := Decripta_Email(trim(tfArquivoIni.ReadString('SISGESTAO','senha','X')));
     ARQUIVOINI.CAMINHOFOTOUSUARIO := tfArquivoIni.ReadString('SISGESTAO','CaminhoFotoUsuario','X');
     ARQUIVOINI.SISGESTAOSERVIDOR  := tfArquivoIni.ReadString('SISGESTAO','SisgestaoServidor','X');
     ARQUIVOINI.EXESERVIDOR        := tfArquivoIni.ReadString('SISGESTAO','ExeServidor','X');
     ARQUIVOINI.CAMINHONFE         := tfArquivoIni.ReadString('SISGESTAO','CaminhoNFE','c:\sisgestao\nfe\');
     ARQUIVOINI.PDF1VIA            := tfArquivoIni.ReadString('SISGESTAO','PDF1VIA','X');
     ARQUIVOINI.LOGERROSBACKUP     := tfArquivoIni.ReadString('SISGESTAO','LogErrosBackup','X');
     ARQUIVOINI.CAMINHONFE         := tfArquivoIni.ReadString('SISGESTAO','CaminhoNfe','X');
     ARQUIVOINI.BKPNFE             := tfArquivoIni.ReadString('SISGESTAO','bkpnfe','X');
     ARQUIVOINI.USUARIO            := tfArquivoIni.ReadString('SISGESTAO','usuario','X');
     ARQUIVOINI.USUARIO            := Decripta_Email(ARQUIVOINI.USUARIO);
     ARQUIVOINI.SENHA              := tfArquivoIni.ReadString('SISGESTAO','senha','X');
     ARQUIVOINI.SENHA              := Decripta_Email(ARQUIVOINI.SENHA);
     ARQUIVOINI.CAMINHOXML         := tfArquivoIni.ReadString('SISGESTAO','CaminhoXML','X');
     ARQUIVOINI.CAMINHORETORNO     := tfArquivoIni.ReadString('SISGESTAO','CaminhoRetorno','X');
     ARQUIVOINI.RETORNOXML         := tfArquivoIni.ReadString('SISGESTAO','RetornaXML','X');
     ARQUIVOINI.CAMINHOSESSAO      := tfArquivoIni.ReadString('SISGESTAO','CaminhoSessao','X');
     ARQUIVOINI.CAMINHOFOTOUSUARIO := tfArquivoIni.ReadString('SISGESTAO','CaminhoFotoUsuario','X');
     ARQUIVOINI.SISGESTAOSERVIDOR  := tfArquivoIni.ReadString('SISGESTAO','SisgestaoServidor','X');
     ARQUIVOINI.EXESERVIDOR        := tfArquivoIni.ReadString('SISGESTAO','ExeServidor','X');
...
begin
     // arquivo ini
     tfArquivoIni.WriteString('SISGESTAO','Servidor',ARQUIVOINI.SERVIDOR);
     tfArquivoIni.WriteString('SISGESTAO','CaminhoBase',ARQUIVOINI.CAMINHOBASE);
     tfArquivoIni.WriteInteger('SISGESTAO','CorFundo',ARQUIVOINI.CORFUNDO);
     tfArquivoIni.WriteString('SISGESTAO','bkpnfe',ARQUIVOINI.BKPNFE);
     tfArquivoIni.WriteString('SISGESTAO','CaminhoXML',ARQUIVOINI.CAMINHOXML);
     tfArquivoIni.WriteString('SISGESTAO','RetornaXML',ARQUIVOINI.RETORNOXML);
     tfArquivoIni.WriteString('SISGESTAO','CaminhoRetorno',ARQUIVOINI.CAMINHORETORNO);
     tfArquivoIni.WriteString('SISGESTAO','CaminhoSessao',ARQUIVOINI.CAMINHOSESSAO);
     tfArquivoIni.WriteInteger('SISGESTAO','resolucao',ARQUIVOINI.RESOLUCAO);
     tfArquivoIni.WriteString('SISGESTAO','usuario',Encripta_Email(ARQUIVOINI.USUARIO));
     tfArquivoIni.WriteString('SISGESTAO','senha',Encripta_Email(ARQUIVOINI.SENHA));
     tfArquivoIni.WriteString('SISGESTAO','CaminhoFotoUsuario',ARQUIVOINI.CAMINHOFOTOUSUARIO);
     tfArquivoIni.WriteString('SISGESTAO','SisgestaoServidor',ARQUIVOINI.SISGESTAOSERVIDOR);
     tfArquivoIni.WriteString('SISGESTAO','ExeServidor',ARQUIVOINI.EXESERVIDOR);
     tfArquivoIni.WriteString('SISGESTAO','CaminhoNFE',ARQUIVOINI.CAMINHONFE);
     tfArquivoIni.WriteString('SISGESTAO','PDF1VIA',ARQUIVOINI.PDF1VIA);
     tfArquivoIni.WriteString('SISGESTAO','LogErrosBackup',ARQUIVOINI.LOGERROSBACKUP);
...
begin
    ...
    FdConnection.Params.Values['Server'] := AllTrim(ARQUIVOINI.SISGESTAOSERVIDOR);
    FdConnection.Params.Values['Database'] := Trim(ARQUIVOINI.CAMINHOBASE);
    FdConnection.Params.Values['User_Name'] := Trim(ARQUIVOINI.USUARIO);
    ...
    try
        ...
        on E: Exception do
        begin
            Informacao(e.Message+ ' Erro no acesso ao BANCO DE DADOS...' + #13 + 'Caminho: ' + Trim(ARQUIVOINI.CAMINHOBASE), 'Aviso...');
...
begin
     ...
     //scnSisGestao.Params.Values['User_Name']     :=AllTrim(ARQUIVOINI.USUARIO);
     ...
     //scnSisGestao.Params.Values['Database']      := Trim (scaminho);
     //scnSisGestao.Params.Values['SQLDialect']    := '3';
     //try
...
begin
     ...
     sCaminhoIni   := sCaminhoExe+'\GestaoComercial.INI';
     ...
     begin // Existe arquivo, nao existe secao...
          tfArquivoIni := TIniFile.Create(sCaminhoIni);
          ARQUIVOINI.CAMINHOBASE := tfArquivoIni.ReadString('SISGESTAO','CaminhoBase','NAOEXISTESECAO');
...
begin
    ...
    begin
        ...
        dtmSisGestao.FdQryGeral.Active := False;
        dtmSisGestao.FdQryGeral.SQL.Text:='';
        dtmSisGestao.FdQryGeral.SQL.Text:='select * from geral_numeros where (empre_codigo = :emprecodigo)';
...
begin
    ...
    FdQryConsulta.SQL.Text := LowerCase(sSQLGravaAuditoria);
...
begin
    FDMySQLDriverLink.VendorLib := ExtractFilePath(Application.ExeName) + 'SYSGESTAO\libmysql.dll';
end;
...
begin
    ...
    dtmSisGestao.FdQryGeralDados.Active := False;
    dtmSisGestao.FdQryGeralDados.SQL.Clear;
    dtmSisGestao.FdQryGeralDados.SQL.Text := 'select * from geral_emp_especificas where (empre_codigo > 0) ';
    dtmSisGestao.FdQryGeralDados.Active := True;
    if not (dtmSisGestao.FdQryGeralDados.FieldByName('empre_codigo').IsNull) then
